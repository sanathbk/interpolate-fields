...
contains
  ...
  !
  subroutine load_scal(io,filename,comm,myid,ng,nh,lo,hi,s,dsdt,time,istep,P0)
    !
    ! reads/writes a restart file
    !
    implicit none
    ...
    integer         , intent(in) :: myid
    integer , intent(in), dimension(3) :: ng,nh,lo,hi
    real(rp), intent(inout), dimension(lo(1)-nh(1):,lo(2)-nh(2):,lo(3)-nh(3):) :: s
    real(rp), intent(inout), dimension(lo(1):,lo(2):,lo(3):) :: dsdt
    real(rp), intent(inout) :: time, P0
    integer , intent(inout) :: istep
    ...
    case('r')
      ...
      !
      call MPI_FILE_GET_SIZE(fh,filesize,ierr)
      good = (product(int(ng(:),MPI_OFFSET_KIND))*2+3)*(storage_size(1._rp)/8)
      if(filesize /= good) then
        if(myid == 0) print*, ''
        if(myid == 0) print*, '*** Simulation aborted due a checkpoint file with incorrect size ***'
        if(myid == 0) print*, '    file: ', filename, ' | expected size: ', good, '| actual size: ', filesize
      ...
      call io_field(io,fh,ng,nh,lo,hi,disp,s)
      call io_field(io,fh,ng,[0,0,0],lo,hi,disp,dsdt)
      ! call io_field(io,fh,ng,nh,lo,hi,disp,drdt)
 
      call MPI_FILE_SET_VIEW(fh,disp,MPI_REAL_RP,MPI_REAL_RP,'native',MPI_INFO_NULL,ierr)
      nreals_myid = 0
      if(myid == 0) nreals_myid = 3
      call MPI_FILE_READ(fh,fldinfo,nreals_myid,MPI_REAL_RP,MPI_STATUS_IGNORE,ierr)
      call MPI_FILE_CLOSE(fh,ierr)
      call MPI_BCAST(fldinfo,3,MPI_REAL_RP,0,comm,ierr)
      time  =      fldinfo(1)
      istep = nint(fldinfo(2))
      P0    =      fldinfo(3)
    case('w')
      ...
      call MPI_FILE_SET_VIEW(fh,disp,MPI_REAL_RP,MPI_REAL_RP,'native',MPI_INFO_NULL,ierr)
      fldinfo = [time,1._rp*istep,P0]
      nreals_myid = 0
      if(myid == 0) nreals_myid = 3
      call MPI_FILE_WRITE(fh,fldinfo,nreals_myid,MPI_REAL_RP,MPI_STATUS_IGNORE,ierr)
      call MPI_FILE_CLOSE(fh,ierr)
    end select
  end subroutine load_scal
  !
