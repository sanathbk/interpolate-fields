...
module mod_initgrid
  ...
  subroutine gridpoint_cluster_two_end(alpha,z0,z)
    ...
    beta = 1.5
    if(alpha /= 0.) then
     ! z = ((tanh(beta*(2.*alpha*z0 - 1.))/tanh(beta)) + 1)/((tanh(beta*(2*alpha - 1))/tanh(beta)) + 1) !For asymmetric cluster
      z = 0.5*(1.+tanh((z0-0.5)*alpha)/tanh(alpha/2.)) !For symmetric cluster
